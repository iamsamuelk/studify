...
- **AI:** Google Gemini — `gemini-3.5-flash-lite` for parsing (falls back to `gemini-3.1-flash-lite` on quota limits), `gemini-3.8-flash` for explanations (falls back to `gemini-3.7-flash` on quota limits)
